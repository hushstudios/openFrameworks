...
ofRectangle ofTrueTypeFont::getStringBoundingBox(const std::string& c, float x, float y, bool vflip) const{
	...
	float minX = x; // std::numeric_limits<float>::max();
    float minY = y; // std::numeric_limits<float>::max();
    float maxX = x; //-std::numeric_limits<float>::max();
    float maxY = y; //-std::numeric_limits<float>::max();
